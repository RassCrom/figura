-- Tighten reverse-mode score validation to match calcReverseScore in gameConfig.ts:
-- full distance points through 2 km, then a stricter exponential falloff.
...
declare
  ...
  v_perfect_radius_km       constant numeric := 2.0;
  v_distance_decay_km       constant numeric := 750.0;
  v_max_scoring_distance_km constant numeric := 5000.0;
begin
  ...
  for v_round in select * from jsonb_array_elements(p_results) loop
    ...
    if p_mode = 'reverse' then
      ...
      if v_distance_km >= v_max_scoring_distance_km then
        ...
        v_location_pts := case
          when v_distance_km <= v_perfect_radius_km then v_reverse_location_points::int
          else round(
